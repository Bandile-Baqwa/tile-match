    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                self.lastSwappedTiles = {self.highlightedTile, newTile}
...
function PlayState:calculateMatches()
    ...
    local matchScore = #match * 50 * tier
    if hasSpecialTile then
        matchScore = matchScore + 300
    end
    ...
    -- if no matches, we can continue playing
        ...
        if #self.lastSwappedTiles == 2 then
            local tile1 = self.lastSwappedTiles[1]
            local tile2 = self.lastSwappedTiles[2]
            ...
            -- tween coordinates between the two so they swap back
            Timer.tween(0.3, {
                [tile1] = {x = tile2.x, y = tile2.y},
                [tile2] = {x = tile1.x, y = tile1.y}
            })

            -- clear the last swapped tiles
            self.lastSwappedTiles = {}
            self.highlightedTile = nil
        end
        self.canInput = true
